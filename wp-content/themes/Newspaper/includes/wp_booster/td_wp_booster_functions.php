...
require_once('td_gutenberg.php'); // wp 5.0(gutenberg) block editor customizations
...
// Here's the point where all plugins should be hooked in!
	...
	<script>
		...
		(function(){
			...
			if (html_jquery_obj.length && (html_jquery_obj.is('.ie8') || html_jquery_obj.is('.ie9'))) {
				...
				var path = '<?php printf( '%1$s', $style_sheet_path ) ?>';
...
if (is_admin()) {
	...
	if ( current_user_can('switch_themes' ) ) {
		...
		require_once( 'wp-admin/panel/td_theme_plugins_setup.php' );

		add_action( 'after_setup_theme', function (){
			td_theme_plugins_setup::get_instance();
		}, 10 );
...
function td_template_include_filter( $wordpress_template_path ) {
	...
	if ( is_single() and ( ( $wordpress_template_path == td_global::$get_template_directory . '/single.php' ) or ( $wordpress_template_path == get_stylesheet_directory() . '/single.php' ) ) ) {
		global $post;
            ...
            add_action( 'after_plugin_row_' . $plugin_slug, function ($plugin_file, $plugin_data, $status){
                echo '
                    <tr class="td-amp-plugin-warning plugin-update-tr">
                        <td colspan="3" class="plugin-update colspanchange">
                            <div class="notice inline notice-warning notice-alt" style="">
                                <p>The <b>tagDiv Mobile Theme</b> now works best with the <b>AMP</b> plugin to provide a complete solution for your content on mobiles. You can find out more <a href="https://tagdiv.com/amp-newspaper-theme/" target="_blank">here</a>.</p>
                            </div>
                        </td>
                    </tr>
                ';
