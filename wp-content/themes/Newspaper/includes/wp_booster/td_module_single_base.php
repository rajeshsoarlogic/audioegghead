...
class td_module_single_base extends td_module {
    ...
    function show_post_pagination() {
    	echo '<!-- post pagination -->' . $this->get_post_pagination();
    }
...
//        ){
        ...
        if (get_post_format($this->post->ID) == 'video') {
            ...
            if (!empty($td_post_video['td_video']) && class_exists( 'td_video_support')) {
    ...
    //Used only for Newsmag
    function get_modified_date() {
        $visibility_class = '';
        if (td_util::get_option('tds_p_show_modified_date') == 'hide') {
            $visibility_class = ' td-visibility-hidden';
        }
	...
	function show_modified_date() {
    	echo '<!-- modified date -->' . $this->get_modified_date();
	}
...
//                        $section_parts = preg_split('/(<p.*>)/U', $content_part_value, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
                        ...
                        foreach ($section_parts as $section_part_index => $section_part_value) {
                            if (!empty($section_part_value)) {
                                ...
                                if (preg_match('/(<p.*>)/U', $section_part_value) === 1) {
                                    ...
                                    if ($tds_inline_ad_paragraph == $p_open_tag_count && td_util::tdc_is_installed()) {
        ...
        //add the top ad
        if (td_util::is_ad_spot_enabled('content_top') && is_single() && td_util::tdc_is_installed()) {
	    ...
	    if ( td_util::tdc_is_installed() ) {
		    if ( td_util::is_ad_spot_enabled( 'content_bottom' ) && is_single() ) {
			    $content = $content . td_global_blocks::get_instance( 'td_block_ad_box' )->render( array( 'spot_id'    => 'content_bottom',
			                                                                                              'spot_title' => $tds_bottom_ad_title
				    ) );
		    } else {
			    if ( $show_inline_ad_at_bottom !== true ) {
				    switch ( $tds_inline_ad_align ) {
					    case 'left':
						    $content = $content . td_global_blocks::get_instance( 'td_block_ad_box' )->render( array( 'spot_id'    => 'content_inline',
						                                                                                              'align'      => 'left',
						                                                                                              'spot_title' => $tds_inline_ad_title
							    ) );
						    break;
    ...
    function show_item_scope_meta() {
    	echo '<!-- meta -->' . $this->get_item_scope_meta();
    }
    ...
    function show_source_and_via() {
    	echo '<!-- source via -->' . $this->get_source_and_via();
    }
    ...
    function show_next_prev_posts() {
    	echo '<!-- next prev -->' . $this->get_next_prev_posts();
    }
    ...
    function show_author_box() {
    	echo '<!-- author box -->' . $this->get_author_box();
    }
