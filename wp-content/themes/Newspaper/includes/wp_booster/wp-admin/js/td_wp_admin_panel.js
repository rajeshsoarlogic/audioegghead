...
jQuery().ready(function() {
    ...
    if ( jQuery('#td_panel_big_form').length ) {
        ...
        //mobile theme amp settings radio controls
        panel_mobile_amp_settings();
    }
});


function tdmHideAmpOptions() {
    jQuery('#all_templates_supported_fieldset').hide();

    var cloudTemplatesPostTypeOption = jQuery('#amp-options-supported_post_types-tdb_templates');
    cloudTemplatesPostTypeOption.attr("disabled", true);
    cloudTemplatesPostTypeOption.parent().append( '<span class="description"> &mdash; Content is not available as AMP.</span>' );
}


// this adds hide/show functionality for amp settings panel boxes on amp settings radio controls
function panel_mobile_amp_settings() {

    var mobileThemeSettingsPanel = jQuery('.td-panel-menu').find('[data-panel="td-mobile-plugin"]');

    // make sure this runs only if the mobile theme panel settings are available
    if ( mobileThemeSettingsPanel.length ) {

        var mobileThemeSettings = jQuery("#td-mobile-plugin"),
            ampSettings = mobileThemeSettings.find('.td-box-row-amp-options'),
            ampPanelBoxes = mobileThemeSettings.find('.td_panel_box_amp_ads, .td_panel_box_amp_analytics'),
            ampSettingsControl = ampSettings.find('.td-box-control-full');

        if ( ! ampSettingsControl.hasClass('tdm_setting_disabled') ) {
            ampSettings.on('click', '.td-radio-control-option', function(event){
                event.preventDefault();

                var $this = jQuery(this),
                    option = $this.data('option-value');

                if ( option === 'mobile_amp' || option === 'amp' ) {
                    ampPanelBoxes.removeClass('tdm_settings_disabled');
                } else {
                    ampPanelBoxes.addClass('tdm_settings_disabled');
                }
...
// automated panel navigation
function panel_navigation() {

    jQuery('.td-panel-menu a').on( 'click', function(event){
...
function td_panel_box() {
     ...
     * normal box
     */
    jQuery('.td-box-header-js-inline').on( 'click', function (event) {
     ...
     */
    jQuery('.td-box-header-js-ajax').on( 'click', function (event) {
...
// used to detect the page load for panel navigation
//var firstLoad = true;
...
function td_ajax_form_submit() {
    ...
    jQuery('#td_button_save_panel').on( 'click', function(event){
...
function td_add_events_mce_for_sidebar() {
    jQuery('.td-sidebar-position-default, .td-sidebar-position-left, .td-no-sidebar, .td-sidebar-position-right').on( 'click', function(e) {
        td_resize_tiny_mce_for_sidebar();
    });
}
    ...
    var tmce = setTimeout(function(){
        ...
        if ( $contentIfr.length ) {
            ...
            if ( 'no_sidebar' === sidebar_position ) {
                ...
                $body.addClass("mceContentBody-max-width-small").removeClass("mceContentBody-max-width-big");
            }
