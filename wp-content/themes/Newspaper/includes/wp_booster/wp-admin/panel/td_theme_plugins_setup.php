...
class td_theme_plugins_setup {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			'theme-plugins-setup',
			td_global::$get_template_directory_uri . '/includes/wp_booster/wp-admin/js/td_theme_plugins_setup.js',
			array( 'jquery' ),
			TD_THEME_VERSION
		);
	...
	private function _get_plugins() {
		...
		foreach ( $instance->plugins as $slug => $plugin ) {
			...
			if ( td_util::is_active( $plugin ) && false === $instance->does_plugin_have_update( $slug ) && false === $this->theme_plugin_has_update( $slug ) ) {
                continue;
			} else {
        ...
        if ( count( $plugins['all'] ) ) {
            ...
            <div class="td-admin-wrap about-wrap theme-browser td-admin-plugins feature-section td-admin-setup-plugins">
                                ...
                                <div class="themes-plugin-status">
                                    <div class="themes-plugin-status-txt">
                                        <?php
                                            ...
                                            if ( isset( $plugins['update'][ $slug ] ) ) { echo 'Outdated <span> / </span>'; }
                ...
                <div class="theme-plugins-installed">
                    <img src="" data-src="<?php echo td_global::$get_template_directory_uri . '/includes/wp_booster/wp-admin/images/plugins/plugins-installed-success.gif'; ?>">
	...
	public function theme_plugin_has_update( $slug ) {
		...
		foreach ( td_global::get_td_plugins() as $constant => $settings ) {
	...
	public function theme_deactivated_plugin( $plugin ) {
		...
		$theme_deactivated_plugin_array = td_options::get_array( 'td_theme_deactivated_plugins' );
		...
		if ( isset( $theme_deactivated_plugin_array[$plugin['slug']] ) ) {
			...
			if ( false === $this->theme_plugin_has_update( $plugin['slug'] ) && td_util::is_active( $plugin ) ) {
				unset( $theme_deactivated_plugin_array[$plugin['slug']] );
				td_options::update_array('td_theme_deactivated_plugins', $theme_deactivated_plugin_array );
			} else {
				return true;
			}
		}
