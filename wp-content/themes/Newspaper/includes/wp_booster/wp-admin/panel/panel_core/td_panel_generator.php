...
class td_panel_generator {
    ...
    static function visual_select_o($params_array) {
        ...
        foreach($params_array['values'] as $list_option) {
            ...
            //!!!! add here for default support $user_data
            ...
            <div id="td-social-place-holder">
                <?php
                    ...
                    foreach ($social_networks as $social_network => $is_enabled) {
                        ?>
                        <div class="td-drag-and-drop-social td-classic-check td-dad-<?php echo esc_attr( $social_network ) ?>">
                            <input type="hidden" name="td_social_drag_and_drop_sort[]" value="<?php echo esc_attr( $social_network ) ?>" />
                            <input type="checkbox" id="td_social_drag_and_drop_<?php echo esc_attr( $i ) ?>" name="td_social_drag_and_drop[]" value="<?php echo esc_attr( $social_network ) ?>" <?php if ($is_enabled) {echo "checked";}?>>
                            <label for="td_social_drag_and_drop_<?php echo esc_attr( $i ) ?>" class="td-check-wrap">
    ...
    */
    ...
    static function radio_button_control($params_array) {
        ...
        foreach($params_array['values'] as $radio_option) {
            ...
            $title = $radio_option['text'];
            if( !empty($radio_option['descr']) ) {
                $title = $radio_option['descr'];
            }

            $buffy .= '
                <a class="td-panel-remove-transitions td-radio-control-option' . $top_option_border_class . $radio_option_border_class_selected . '" title="' . $title . '" data-control-id="' . $contro_unique_id . '" data-option-value="' . $radio_option['val'] . '">' . $display_option . '<span class="td-radio-check"></span></a>
            ';
            $icont++;
    ...
    static function color_picker($params_array) {
        ...
        ?>
        <input type="text" id="<?php echo esc_attr( $control_unique_id ) ?>" value="<?php echo esc_attr( $control_value ) ?>" data-default-color="<?php echo esc_attr( $params_array['default_color'] ) ?>" name="<?php echo esc_attr( self::generate_name($params_array) ) ?>" />
        <input type="hidden" name="<?php echo self::generate_default_values_name($params_array) ?>" value="<?php echo esc_attr( $params_array['default_color'] ) ?>">
        <script>
            jQuery("#<?php printf( '%1$s', $control_unique_id ) ?>").cs_wpColorPicker();
        </script>
        <?php
        $buffy = ob_get_clean();
	...
	static function css_editor($params_array) {
		...
		<div class="td-code-editor-wrap td-css-editor-wrap">
			<textarea class="td-code-editor-textarea <?php echo esc_attr( $editor_text_uid ) ?>" name="<?php echo self::generate_name($params_array) ?>"><?php printf( '%1$s', $control_value ) ?></textarea>
			<div id="<?php echo esc_attr( $editor_uid ) ?>" class="td-code-editor" <?php printf( '%1$s', $td_css_inline->get_inline_css() ) ?>></div>
			<div class="td-clear-fix"></div>
		</div>
		<script>
			(function (){
				var editor_textarea = jQuery('.<?php printf( '%1$s', $editor_text_uid ) ?>');
				ace.require("ace/ext/language_tools");
				var editor = ace.edit("<?php printf( '%1$s', $editor_uid ) ?>");
    ...
    static function box_start($panel_name = '', $is_open = true, $custom_class = false ) {
        ...
        if ( ! empty ( $panel_name ) ) {
                        $buffy .= '<div class="td-box-header td-box-header-js-inline" data-box-id="' . $box_uid . '" unselectable="on">
        ...
        $buffy .= '<div class="td-box-content-wrap">
    ...
    static function ajax_box($panel_text, $ajax_params = array(), $custom_unique_id = '', $panel_class = '') {
        ...
        $buffy = '
        <div class="td-box td-box-close ' . sanitize_html_class( strtolower( str_replace( array( ' ', '-' ), '_', $panel_class ) ) ) . '" id="' . $box_uid . '">
