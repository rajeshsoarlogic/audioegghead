...
function td_deactivate_old_plugins() {
	...
	foreach ( td_global::get_td_plugins() as $constant => $settings ) {
		...
		foreach ( td_global::$theme_plugins_list as $theme_plugin ) {
			if ( $theme_plugin['td_class'] === $settings['class'] ) {
				$config_plugin_data_array = $theme_plugin;
			}
		}

		if ( ! empty( $config_plugin_data_array ) && td_util::is_active( $config_plugin_data_array ) ) {
			$plugin_version = null;
			if ( defined( $constant ) ) {
	...
	if ( !empty( $td_db_store_deactivated_plugins ) ) {
		...
		$theme_deactivated_plugins_array = td_options::get_array( 'td_theme_deactivated_plugins' );

		if ( empty( $theme_deactivated_plugins_array ) ) {
			td_options::update_array('td_theme_deactivated_plugins', $td_db_store_deactivated_plugins );
		} else {
			td_options::update_array('td_theme_deactivated_plugins', array_merge( $theme_deactivated_plugins_array, $td_db_store_deactivated_plugins ) );
		}
