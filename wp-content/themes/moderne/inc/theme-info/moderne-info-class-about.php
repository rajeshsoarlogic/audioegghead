 ...
 * @subpackage Admin
 */
if ( ! class_exists( 'moderne_info_page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class moderne_info_page {
		/**
		 * Define the version of the class.
		 * @var string $version The moderne_info_page class version.
		 */
		...
		/**
		 * The single instance of moderne_info_page
		 * @var moderne_info_page $instance The  moderne_info_page instance.
		 */
		private static $instance;

		/**
		 * The Main moderne_info_page instance.
		 * We make sure that only one instance of moderne_info_page exists in the memory at one time.
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof moderne_info_page ) ) {
				self::$instance = new moderne_info_page;
		...
		function register() {
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				add_theme_page( $this->menu_name, $this->page_name, 'activate_plugins', 'moderne-welcome', array(
					$this,
					'page_render',
				) );
			}
		...
		public function welcome_admin_notice() {
			...
			if ( ! empty( $this->notification ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) && !get_option( 'moderne_notice_welcome' ) && 'themes.php' == $pagenow ) {

					echo '<div class="moderne-info-about updated notice is-dismissible">';
					echo '<a class="notice-dismiss" href="' . esc_url(wp_nonce_url(remove_query_arg(array('activated'), add_query_arg('moderne-hide-notice', 'welcome')), 'moderne_hide_notices_nonce', '_moderne_notice_nonce')) . '" style="z-index: 0;padding: 10px;text-decoration: none;" >';
					echo '<span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'moderne'). '</span>';
					echo '</a>';
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				if ( ! empty( $this->tabs ) ) {
					...
					foreach ( $this->tabs as $tab_key => $tab_name ) {
						...
						echo '<a href="' . esc_url( admin_url( 'themes.php?page=moderne-welcome' ) ) . '&tab=' . esc_attr( $tab_key ) . '" class="nav-tab ' . ( $active_tab == $tab_key ? 'nav-tab-active' : '' ) . '" role="tab" data-toggle="tab">';
		...
		public function getting_started() {
			...
			if ( ! empty( $this->config['getting_started'] ) ) {
				...
				if ( ! empty( $getting_started ) ) {
					...
					echo '<div id="moderne-getting-started" class="feature-section three-col">';
		...
		public function support_content() {
			echo '<div id="moderne-support-content" class="feature-section two-col">';
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					echo '<div id="moderne-free-pro" class="feature-section">';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						echo '<tr class="moderne-free-pro-button">';
						echo '<td></td>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			// enqueue css files
			if ( 'themes.php' === $pagenow || 'appearance_page_moderne-welcome' == $hook_suffix ) {
				wp_enqueue_style( 'moderne-about-page-css', get_template_directory_uri() . '/inc/theme-info/css/moderne-info-backend.css' );
			}

			// enqueue js files
			if ( 'appearance_page_moderne-welcome' == $hook_suffix ) {
				wp_enqueue_script( 'moderne-about-page-js', ( get_template_directory_uri() . '/inc/theme-info/js/moderne-info-backend.js' ), array( 'jquery' ), '', 'true' );
			}
		...
		public function hide_notice() {
			if (isset($_GET['moderne-hide-notice']) && isset($_GET['_moderne_notice_nonce'])) {
				if (!wp_verify_nonce($_GET['_moderne_notice_nonce'], 'moderne_hide_notices_nonce')) {
					wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'moderne'));
				}
				if (!current_user_can('edit_theme_options')) {
					wp_die(esc_html__('You do not have the necessary permission to perform this action.', 'moderne'));
				}
				$hide_notice = sanitize_text_field( wp_unslash($_GET['moderne-hide-notice']));
				update_option('moderne_notice_' . $hide_notice, 1);
			}
		}
