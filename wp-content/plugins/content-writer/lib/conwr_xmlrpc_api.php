...
if (!function_exists("conwr_new_xmlrpc_methods")) {
	function conwr_new_xmlrpc_methods($methods) {
		...
		$methods['conwr.PluginUpdate'] = 'conwr_xmlrpc_update_plugin';
...
if (!function_exists("conwr_xmlrpc_update_plugin")) {
	function conwr_xmlrpc_update_plugin($data) {
		if (!conwr_check_api_key()) {
			return json_encode(array('Status' => "Failed", 'Error'  => "Plugin is not verified."));
		}
		...
		if (strpos($check_response, "ok:") !== false) {
			...
			return json_encode(array('Status' => "Successful", 'Description' => "Plugin updated to V{$version}", 'CurrentVersion' => $version));
		}
		else if (strpos($check_response, "up to date") !== false) {
			$version = str_replace("up to date:", "", $check_response);
			return json_encode(array('Status' => "Successful", 'Description' => "Plugin is up to date.", 'CurrentVersion' => $version));
		}
		else if (strpos($check_response, "error:") !== false) {
			return json_encode(array('Status' => "Failed", 'Error' => "Error: " . str_replace("error:", "", $check_response)));
		}
	}
}
