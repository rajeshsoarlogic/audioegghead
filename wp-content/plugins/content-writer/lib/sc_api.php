...
if (!function_exists("conwr_rest_api_init")) {
	function conwr_rest_api_init() {
		...
		register_rest_route('sc-api/v1', '/PluginUpdate', array('methods' => 'POST', 'callback' => 'conwr_update_plugin'));
...
if (!function_exists("conwr_rest_validate_token")) {
	function conwr_rest_validate_token($data, $function_name = "") {
        ...
        $post_data = json_decode( urldecode(decode_value_for_plugin($json_request)));
...
if (!function_exists("conwr_create_post_preview")) {
	function conwr_create_post_preview($data) {
		...
		if (isset($post_data) && !empty($post_data)) {
			...
			conwr_api_write_log("Error: Post data is empty. Function conwr_create_post_preview; ");
		}
...
if (!function_exists("conwr_update_plugin")) {
	function conwr_update_plugin($data) {
		if (!conwr_check_api_key()) {
			return array('Status' => "Failed", 'Error'  => "Plugin is not verified.");
		}
		...
		$check_response = conwr_check_updates();

		if (strpos($check_response, "ok:") !== false) {
			$version = str_replace("ok:", "", $check_response);
			return array('Status' => "Successful", 'Description' => "Plugin updated to V{$version}", 'CurrentVersion' => $version);
		}
		else if (strpos($check_response, "up to date") !== false) {
			$version = str_replace("up to date:", "", $check_response);
			return array('Status' => "Successful", 'Description' => "Plugin is up to date.", 'CurrentVersion' => $version);
		}
		else if (strpos($check_response, "error:") !== false) {
			return array('Status' => "Failed", 'Error' => "Error: " . str_replace("error:", "", $check_response));
		}
	}
}
