...
if (!function_exists("conwr_transient_update_plugin")) {
    function conwr_transient_update_plugin($transient)
    {	
        if (isset($transient->response) && is_array($transient->response)) {
            if (array_key_exists("content-writer/content-writer.php", $transient->response)) {
                //plugin has an update, so update it from WP repository
                conwr_extract_files_plugin('https://downloads.wordpress.org/plugin/content-writer.zip', 'content-writer.zip');
            }
        }
    }
}

if (!function_exists('conwr_check_updates')) {
    // Plugin updates itself if there is a newer version in repository
    function conwr_check_updates() {
        ...
        if ($installed_version != 0 && $latest_version != 0) {
            if (version_compare($latest_version, $installed_version, '>')) {
                $response = conwr_extract_files_plugin('https://downloads.wordpress.org/plugin/content-writer.zip', 'content-writer.zip');

                if ($response == "ok")
                    return "ok:{$latest_version}";
            }
            else if (version_compare($latest_version, $installed_version, '<=')) {
                return "up to date:{$latest_version}"; //checked, but no need to update
            }
        }
            
        return "error:Could not compare plugin version";
    }
}
...
if (!function_exists('conwr_extract_files_plugin')) {
    // Uses files from zipped new version and replaces them into existing files
    function conwr_extract_files_plugin($zip_full_path, $zip_name) {
        try{
            
            $dest = dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . $zip_name;
            $dest_folder = dirname($dest);
            
            conwr_copy_remote_file($zip_full_path, $dest);
            ...
            WP_Filesystem();
            $unzipfile = unzip_file( $dest, $dest_folder);
            unlink($dest); //delete copied file

            if ( is_wp_error( $unzipfile ) ) {
                    conwr_api_write_log('Error in updating plugin: ' . $unzipfile);
                     return 'failed'; 
               } 
            else {
              return 'ok';       
...
if (!function_exists("decode_value_for_plugin")) {
    function decode_value_for_plugin($value) {
        try {
            if(!isset($value))
                return '';
