...
class tdb_single_related extends td_block {
    ...
    function render( $atts, $content = null ) {
        ...
        $atts['sample_posts_data']       = $post_related_data['sample'];
        ...
        // when we have no related posts to display
        if ( $this->td_query->post_count == 0 ) {
            if ( tdc_state::is_live_editor_iframe() || tdc_state::is_live_editor_ajax() ) {

                $post_related_data['sample'] = true;
                $post_related_data['sample_posts'] = $post_related_data['dummy_data_array']['sample_posts'];
        ...
        // when the post has no tags but the related posts filter is set to show the related posts by tags
        if ( isset( $post_related_data['sample_no_tags'] ) and $post_related_data['sample_no_tags'] === true ) {
        ...
        $buffy .= '<div class="' . $this->get_block_classes($additional_classes_array) . ' tdb-single-related-posts" ' . $this->get_block_html_atts() . '>';
            ...
            $buffy .= '<div id=' . $this->block_uid . ' class="td_block_inner tdb-block-inner td-fix-index">';
                ...
                if ( $post_related_data['sample'] ) {
                    $buffy .= $this->inner( $post_related_data['sample_posts'], true ); //inner content of the block
                } else {
    ...
    function inner( $posts, $sample = false ) {
        ...
        if ( !empty( $posts ) ) {
            if ( $sample ) {
                foreach ( $posts as $post ) {
                    $tdb_module_related_sample = new tdb_module_related_sample( $post, $this->get_all_atts() );
                    $buffy .= $tdb_module_related_sample->render( $post );
                }
            } else {
                foreach ( $posts as $post ) {
